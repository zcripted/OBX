...
public class ModerationService {
    ...
    public void load() {
        if (!store.isAvailable()) {
            plugin.getLogger().severe("[SF-Core] ModerationService disabled — SQLite store unavailable.");
            return;
        }
    ...
    private void migrateLegacyYaml() {
        ...
        plugin.getLogger().info("[SF-Core] Migrated " + profiles + " moderation profile(s), " + mutes +
    ...
    private void renameMigrated(File legacy) {
        ...
        if (!legacy.renameTo(renamed)) {
            plugin.getLogger().warning("[SF-Core] Could not rename moderation.yml after migration.");
        }
    }
    ...
    private UUID resolveOrSynthesizeUuid(String name, String rawUuid) {
        ...
        return UUID.nameUUIDFromBytes(("sf-core:" + name.toLowerCase(Locale.ENGLISH)).getBytes(StandardCharsets.UTF_8));
    }
    ...
    public boolean unban(String playerName, String actor, String reason) {
        ...
        if (resolved == null) {
            resolved = new ResolvedProfile(effectiveTarget,
                    UUID.nameUUIDFromBytes(("sf-core:" + effectiveTarget.toLowerCase(Locale.ENGLISH)).getBytes(StandardCharsets.UTF_8)),
                    false, null);
        }
    ...
    private ResolvedProfile resolveStatusProfile(String input) {
        ...
        return new ResolvedProfile(activeBan.getTarget(),
                UUID.nameUUIDFromBytes(("sf-core:" + activeBan.getTarget().toLowerCase(Locale.ENGLISH)).getBytes(StandardCharsets.UTF_8)),
                false, null);
    }
