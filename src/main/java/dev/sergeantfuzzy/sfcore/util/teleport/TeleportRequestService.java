package dev.sergeantfuzzy.sfcore.util.teleport;
...
public final class TeleportRequestService {
    ...
    public void send(Player requester, Player target) {
        ...
        String accept = languages.get(target, "teleport.request.accept-button");
        String deny = languages.get(target, "teleport.request.deny-button");
        ...
        String acceptHover = languages.get(target, "teleport.request.accept-hover");
        String denyHover = languages.get(target, "teleport.request.deny-hover");
        ...
        parts.add(ComponentMessenger.InteractiveMessagePart.interactive(accept,
                Collections.singletonList(acceptHover), "/tpaccept " + requester.getName(), true));
        parts.add(ComponentMessenger.InteractiveMessagePart.plain(" " + or + " "));
        parts.add(ComponentMessenger.InteractiveMessagePart.interactive(deny,
                Collections.singletonList(denyHover), "/tpdeny " + requester.getName(), true));
