...
public final class AdventureMessageUtil {
     ...
     * SF-Core boot banner). Hover and click annotations are dropped since they cannot be
    ...
    private static void applyInteractiveOnly(TextComponent component, Span span) {
        if (span.hoverLines != null && !span.hoverLines.isEmpty()) {
            ...
            // message body — tags like <gold>, <bold>, <gradient:...>, plus
    ...
    static List<Span> parseToSpans(String input) {
        ...
        while (i < len) {
            ...
            if (c == '<') {
                // Quote-aware close-bracket search. {@code <hover:show_text:'<gold>...'>}
     ...
     * contain markup like {@code 'show_text:<gold>X</gold>'} where a naive
     * {@code indexOf('>')} would terminate the tag at the {@code >} of
     * {@code <gold>}). Returns {@code -1} if the tag never closes.
     */
