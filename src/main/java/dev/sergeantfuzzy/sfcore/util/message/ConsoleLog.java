...
/**
 * Centralized, themed console logger for SF-Core.
 *
 ...
 * {@code [SF-Core] [SF-Core]} prefix that resulted from emitting a literal
 * {@code [SF-Core]} tag through the Bukkit {@code PluginLogger} (which already
 * prepends the plugin name itself). Output is written through the
 * {@link ConsoleCommandSender} instead of the plugin logger, so:
 *
 * <ul>
 *   <li>the prefix is rendered exactly once and in SF-Core's gold/aqua palette
 *       (matching the startup banner);</li>
...
public final class ConsoleLog {
    ...
    // ── SF-Core console palette (legacy section-codes) ──────────────────────────
    private static final char S = ChatColor.COLOR_CHAR;
    private static final String BRACKET = S + "8";  // dark grey  — [ ] frames
    private static final String BRAND   = S + "6";  // gold       — SF-Core
    private static final String TAGGED  = S + "b";  // aqua       — subsystem tag
    ...
    /** Info line with no subsystem tag: {@code [SF-Core] message}. */
    public static void info(Plugin plugin, String message) {
        send(plugin, Level.INFO, prefix(null) + BODY + message + RESET, message);
    }

    /** Info line tagged with a subsystem: {@code [SF-Core][Arcanum] message}. */
    ...
    private static String prefix(String tag) {
        StringBuilder builder = new StringBuilder();
        builder.append(BRACKET).append('[').append(BRAND).append("SF-Core").append(BRACKET).append(']');
