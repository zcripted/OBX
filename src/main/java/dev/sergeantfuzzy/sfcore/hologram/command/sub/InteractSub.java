...
public final class InteractSub implements HoloSubCommand {
    ...
    public String permission() {
        return "sfcore.holo.interact";
    }

    @Override
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage("§6/holo interact §f<id> §7<enable|disable|cooldown|width|height> [value]");
            return true;
        }
        ...
        switch (op) {
            ...
            case "cooldown":
                if (args.length < 3) {
                    sender.sendMessage("§6/holo interact §f<id> cooldown <ms>");
                    return true;
                }
                try {
            ...
            case "width":
                if (args.length < 3) {
                    sender.sendMessage("§6/holo interact §f<id> width <blocks>");
                    return true;
                }
                try {
            ...
            case "height":
                if (args.length < 3) {
                    sender.sendMessage("§6/holo interact §f<id> height <blocks>");
                    return true;
                }
                try {
