...
/** {@code /holo seethrough <id> <true|false>} */
public final class SeethroughSub implements HoloSubCommand {

    private final HoloContext ctx;

    public SeethroughSub(HoloContext ctx) {
        this.ctx = ctx;
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage("§6/holo seethrough §f<id> §7<true|false>");
            return true;
        }
        ...
        hologram.getSettings().setSeeThrough(Boolean.parseBoolean(args[1]));
