package dev.sergeantfuzzy.sfcore.messaging;
...
public class MessageService {
    ...
    public void load() {
        if (!store.isAvailable()) {
            plugin.getLogger().warning("[SF-Core] MessageService disabled — SQLite store unavailable.");
            return;
        }
    ...
    private void migrateLegacyYaml() {
        ...
        plugin.getLogger().info("[SF-Core] Migrated " + ignoresMigrated + " ignore row(s) and " + mailMigrated + " mail row(s) from messaging.yml.");
        renameMigrated(legacy);
    }
