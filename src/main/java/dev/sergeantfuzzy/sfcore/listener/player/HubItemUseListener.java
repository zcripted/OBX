...
import dev.sergeantfuzzy.sfcore.hub.item.HubItems;
 ...
 * SF-Core hub items, regardless of the item's material. The single exception is
...
public final class HubItemUseListener implements Listener {
    ...
    public HubItemUseListener(Main plugin, HubService hub) {
        this.plugin = plugin;
        this.hub = hub;
    }
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        // This is an SF-Core hub item, so claim the interaction. We run at LOWEST
        ...
        switch (id) {
            case HubItems.ID_SERVER_SELECTOR:
                if (!player.hasPermission("sfcore.hub.selector")) {
            ...
            case HubItems.ID_VANISH_ALL:
                if (!player.hasPermission("sfcore.hub.vanishall")) {
                    plugin.getLanguageManager().send(player, "core.no-permission");
                    return;
                }
            ...
            case HubItems.ID_LAUNCHPAD:
                ...
                if (hub.isItemEnabled(HubItems.ID_LAUNCHPAD) && player.hasPermission("sfcore.hub.launchpad")) {
            ...
            case HubItems.ID_JUMP_ROD:
                ...
                if (!player.hasPermission("sfcore.hub.jumprod")) {
                    event.setCancelled(true);
                    denyVanillaUse(event);
                    plugin.getLanguageManager().send(player, "core.no-permission");
                }
        ...
        for (Player other : Bukkit.getOnlinePlayers()) {
            ...
            try {
                if (nextHidden) {
                    ...
                    if (vanishManager != null && vanishManager.isVanished(other) && !player.hasPermission("sfcore.vanish")) {
            ...
            for (Player target : Bukkit.getOnlinePlayers()) {
                ...
                try {
                    if (vanishManager != null && vanishManager.isVanished(target)
                            && !viewer.hasPermission("sfcore.vanish")) {
