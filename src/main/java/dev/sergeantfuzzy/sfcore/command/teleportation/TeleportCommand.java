 ...
 * {@code sfcore.teleport.admin}. Console can move players between each other but has no
...
public final class TeleportCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("sfcore.teleport.admin")) {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (!sender.hasPermission("sfcore.teleport.admin") || args.length == 0 || args.length > 2) {
