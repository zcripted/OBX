...
import dev.sergeantfuzzy.sfcore.kit.Kit;
import dev.sergeantfuzzy.sfcore.kit.KitService;
...
public class KitCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("sfcore.kit")) {
    ...
    private boolean list(CommandSender sender) {
        if (!sender.hasPermission("sfcore.kit.list") && !sender.hasPermission("sfcore.kit")) {
        ...
        for (Kit kit : all) {
            if (sender instanceof Player) {
                Player player = (Player) sender;
                if (!player.hasPermission("sfcore.kit." + kit.getName().toLowerCase())
                        && !player.hasPermission("sfcore.kit.*")) {
                    continue;
                }
            }
    ...
    private boolean info(CommandSender sender, String[] args) {
        if (!sender.hasPermission("sfcore.kit.info")) {
            languages.send(sender, "core.no-permission");
            return true;
        }
        if (args.length < 2) {
    ...
    private boolean give(CommandSender sender, String[] args) {
        if (!sender.hasPermission("sfcore.kit.give")) {
            languages.send(sender, "core.no-permission");
            return true;
        }
        if (args.length < 3) {
    ...
    private boolean reload(CommandSender sender) {
        if (!sender.hasPermission("sfcore.kit.reload")) {
            languages.send(sender, "core.no-permission");
            return true;
        }
        kitService.reload();
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            ...
            options.add("list");
            if (sender.hasPermission("sfcore.kit.info")) options.add("info");
            if (sender.hasPermission("sfcore.kit.give")) options.add("give");
            if (sender.hasPermission("sfcore.kit.reload")) options.add("reload");
            for (Kit kit : kitService.getKits()) {
                if (sender instanceof Player && !((Player) sender).hasPermission("sfcore.kit." + kit.getName().toLowerCase())
                        && !((Player) sender).hasPermission("sfcore.kit.*")) continue;
