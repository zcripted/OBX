package dev.sergeantfuzzy.sfcore.hub.item;
 ...
 *       via reflection — keyed under {@code sfcore:hub_item_id}.</li>
...
public final class HubItems {
    ...
    private static NamespacedKey idKey(Main plugin) {
        return new NamespacedKey(plugin, "hub_item_id");
    }

    private static NamespacedKey stateKey(Main plugin) {
     ...
     */
    public static String getId(Main plugin, ItemStack stack) {
    ...
    public static String getState(Main plugin, ItemStack stack) {
        if (stack == null || !stack.hasItemMeta()) {
            return null;
        }
        ItemMeta meta = stack.getItemMeta();
    ...
    public static boolean isHubItem(Main plugin, ItemStack stack) {
        return getId(plugin, stack) != null;
    }

    // ── Builders ───────────────────────────────────────────────────────

    public static ItemStack buildServerSelector(Main plugin, HubService hub) {
        ...
        return buildHubItem(plugin, material,
                hub.itemName("items.server-selector", "&6&lServer Selector"),
                hub.itemLore("items.server-selector"),
                ID_SERVER_SELECTOR, null);
    }

    public static ItemStack buildJumpRod(Main plugin, HubService hub) {
        ...
        return buildHubItem(plugin, material,
                hub.itemName("items.jump-rod", "&b&lJump-To Rod"),
                hub.itemLore("items.jump-rod"),
                ID_JUMP_ROD, null);
    }

    public static ItemStack buildVanishAll(Main plugin, HubService hub, boolean playersVisible) {
    ...
    public static ItemStack buildLaunchpad(Main plugin, HubService hub) {
    ...
    private static ItemStack buildHubItem(Main plugin, Material material, String name, List<String> lore,
