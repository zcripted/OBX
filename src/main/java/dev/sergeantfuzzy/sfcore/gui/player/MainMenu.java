...
public final class MainMenu {
    ...
    public static void open(Main plugin, Player player) {
        ...
        String title = ChatColor.GOLD.toString() + ChatColor.BOLD + "Main "
        ...
        if (coreMeta != null) {
            coreMeta.setDisplayName(ChatColor.GOLD + "SF-Core");
            coreMeta.setLore(Arrays.asList(
                    ChatColor.YELLOW + "Developer: " + ChatColor.WHITE + "SergeantFuzzy",
        ...
        ItemStack infoPlaceholder = createPane(ChatColor.YELLOW + "Coming Soon", Arrays.asList(
                ChatColor.GRAY + "This slot is reserved for a future",
                ChatColor.GRAY + "feature. Dragging and clicks are disabled."
        ));
        for (int slot : INFORMATION_SLOTS) {
            if (slot == WARP_ITEM_SLOT) {
                continue;
            }
            inventory.setItem(slot, infoPlaceholder.clone());
        }

        player.openInventory(inventory);
    }

    private static ItemStack createPlaceholderItem() {
        return createPane(ChatColor.YELLOW + "Coming Soon", Arrays.asList(
                ChatColor.GRAY + "This slot is reserved for a future",
                ChatColor.GRAY + "feature. Dragging and clicks are disabled."
        ));
    }
