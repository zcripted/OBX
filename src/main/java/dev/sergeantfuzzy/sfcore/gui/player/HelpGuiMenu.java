...
/**
 * Paginated help GUI for the SF-Core /help override.
 *
...
public final class HelpGuiMenu {
    ...
    private static final String SF_CORE_COMMAND_PACKAGE = "dev.sergeantfuzzy.sfcore.command.";
    private static final String SF_CORE_PLUGIN_NAME = "SF-Core";

    /**
     * Authoritative mapping of every SF-Core command to its display category. Keyed by
     ...
     * {@code dev.sergeantfuzzy.sfcore.command.admin} → {@code dev.sergeantfuzzy.sfcore.b.a})
    ...
    static {
        ...
        // Core
        map.put("sf", CATEGORY_CORE);
        map.put("sfcore", CATEGORY_CORE);
        map.put("sfc", CATEGORY_CORE);
     ...
     *   each declared alias. This is authoritative for SF-Core commands and survives
     *   ProGuard's package renaming.</li>
     *   <li>If the registered command originates from the SF-Core plugin (matched by the
     *   owning plugin's name), the executor package is inspected as a fallback in case a
     *   future SF-Core command is added without updating the map.</li>
    ...
    private static String deriveCategory(String commandName, PluginCommand command) {
        ...
        if (command != null) {
            ...
            // Best-effort fallback for commands registered by SF-Core that aren't in the map
            // yet. We treat any command whose owning plugin reports name "SF-Core" as ours
            // and bucket it under the most generic SF-Core category (Core).
            try {
