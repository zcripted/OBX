...
public class NicknameService {
    ...
    public void load() {
        if (!store.isAvailable()) {
            plugin.getLogger().warning("[SF-Core] NicknameService disabled — SQLite store unavailable.");
            return;
        }
    ...
    private void applyToPlayer(Player player, String displayName) {
        ...
        try {
            player.setPlayerListName(displayName.length() <= 16 ? displayName : displayName.substring(0, 16));
