...
public final class BoardSub implements HoloSubCommand {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        switch (action) {
            ...
            case "block":
                if (args.length < 3) {
                    sender.sendMessage(HoloMessages.inline("§5/holo board §f<id> material §7<MATERIAL>"));
                    return true;
                }
            ...
            case "scale":
                if (args.length < 3) {
                    sender.sendMessage(HoloMessages.inline("§5/holo board §f<id> size §7<width> §8[height|auto]"));
                    return true;
                }
                double width;
                try {
            ...
            case "back":
                if (args.length < 3) {
                    sender.sendMessage(HoloMessages.inline("§5/holo board §f<id> offset §7<distance>"));
                    return true;
                }
                double back;
                try {
