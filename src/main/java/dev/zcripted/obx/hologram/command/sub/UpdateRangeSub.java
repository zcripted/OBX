...
/** {@code /holo updaterange <id> <blocks>} */
public final class UpdateRangeSub implements HoloSubCommand {

    private final HoloContext ctx;

    public UpdateRangeSub(HoloContext ctx) {
        this.ctx = ctx;
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage("§5/holo updaterange §f<id> <blocks>");
            return true;
        }
        ...
        try {
            hologram.getSettings().setUpdateRange(Double.parseDouble(args[1]));
