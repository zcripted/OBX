...
public class AutoResourcePackManager {
    ...
    public void installBundledPack() {
        ...
        try (InputStream bundled = plugin.getResource("bundled-resourcepack/obx-gui-pack.zip")) {
            if (bundled == null) {
    ...
    public void prepareHosting() {
        ...
        if (packUrl != null) {
            dev.zcripted.obx.util.message.ConsoleLog.info(plugin, "Resource pack URL set to " + packUrl);
        } else {
    ...
    private String normalizeConfiguredUrl(String configured) {
        ...
        String url = hasPath ? "http://" + value : "http://" + value + "/obx-core-pack.zip";
        lastComputedUrl = url;
        return url;
    }
