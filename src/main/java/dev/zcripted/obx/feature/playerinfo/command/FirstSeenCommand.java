...
public class FirstSeenCommand extends AbstractObxCommand implements TabCompleter {

    private final PlaytimeService playtime;

    public FirstSeenCommand(ObxPlugin plugin) {
        super(plugin);
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("obx.firstseen")) {
            languages.send(sender, "core.no-permission");
            return true;
        }
        if (args.length < 1) {
            languages.send(sender, "info.firstseen.usage");
            return true;
        }
        ...
        String firstSeen = playtime.getFirstSeen(target.getUniqueId());
        if (firstSeen == null) {
            languages.send(sender, "info.firstseen.unknown", Placeholders.with("player", target.getName()));
            return true;
        }
        languages.send(sender, "info.firstseen.line",
                Placeholders.with("player", target.getName(), "firstSeen", firstSeen));
        return true;
    }
