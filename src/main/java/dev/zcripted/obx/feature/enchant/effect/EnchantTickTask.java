...
public final class EnchantTickTask implements Runnable {
    ...
    public void start() {
        stop();
        // Route through SchedulerAdapter so this works on Folia (the global
        // BukkitScheduler doesn't exist there). Per-player work is then dispatched
        // onto each player's own region thread in run().
        task = plugin.getSchedulerAdapter().runRepeating(this, PERIOD_TICKS, PERIOD_TICKS);
    }
    ...
    public void run() {
        ...
        for (Player player : plugin.getServer().getOnlinePlayers()) {
            if (folia) {
                // Touching a player's inventory/potions/velocity must run on that
                // player's region thread under Folia.
                scheduler.runAtEntity(player, () -> safeTick(player));
            } else {
                safeTick(player);
            }
        }
    ...
    private void safeTick(Player player) {
        try {
            tickPlayer(player);
        } catch (Throwable ignored) {
