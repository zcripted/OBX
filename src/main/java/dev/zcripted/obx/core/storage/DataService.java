...
public class DataService {
    ...
    public void load() {
        if (!store.isAvailable()) {
            plugin.getLogger().severe("DataService disabled — SQLite store unavailable.");
            return;
        }
    ...
    private void migrateLegacyYaml() {
        ...
        plugin.getLogger().info("Migrated " + spawn + " spawn row(s), " + backs + " back row(s), " +
        ...
        if (!legacy.renameTo(renamed)) {
            plugin.getLogger().warning("Could not rename data.yml after migration.");
        }
    }
