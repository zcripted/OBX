...
* New package root `dev.zcripted.obx.hologram` with submodules
  `backend/`, `model/`, `packet/`, `service/`.
* `src/main/java/dev/zcripted/obx/hologram/model/HologramId.java` —
...
* `src/main/java/dev/zcripted/obx/hologram/model/Hologram.java` —
...
* `src/main/java/dev/zcripted/obx/hologram/model/HologramLine.java`
...
* `src/main/java/dev/zcripted/obx/hologram/backend/HologramBackend.java`
...
* `src/main/java/dev/zcripted/obx/hologram/backend/BackendSelector.java`
...
* `src/main/java/dev/zcripted/obx/hologram/service/HologramService.java`
  ...
  selection.
* `src/main/java/dev/zcripted/obx/hologram/service/HologramRegistry.java`
...
* `src/main/java/dev/zcripted/obx/Main.java` — three additive blocks:
...
* `src/main/resources/plugin.yml` — registers `/sfholo` (aliases `sfholograms`,
  `sfh`) under permission `obx.holo.use`. The executor is bound in
  Phase 1 / Phase 2 — until then the command resolves to Bukkit's default
  "unknown command" reply, which is harmless.
* `src/main/resources/plugin.yml` — adds the `obx.holo.*` permission
  tree (use, admin, create, delete, edit, list, info, tp, interact, gui)
  plus inclusion under the existing `obx.*` umbrella so a single grant
...
* `mvn -DskipTests package` — green up to and including the maven-jar step
  (`target/OBX-1.0.0-SNAPSHOT.jar` produced). The exec-maven-plugin
  ...
  * `dev/zcripted/obx/hologram/{backend,model,service,packet}/*.class`
...
```
OBX Holograms: Phase 0 — bootstrap module skeleton + dormant service
```
