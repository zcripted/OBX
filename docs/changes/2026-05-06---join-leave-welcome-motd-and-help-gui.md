...
Both modules can be flipped at runtime via `/sf joinleave on|off|status` and `/sf joinmotd on|off|status` (no plugin reload required) and via the `join-leave.enabled` / `join-motd.enabled` keys in `config.yml`.

## Categories

### Commands
- Added `/sf joinleave <on|off|status>` — toggles the join/leave broadcast module. Permission: `sfcore.admin.modules.joinleave`.
- Added `/sf joinmotd <on|off|status>` — toggles the in-game welcome MOTD module. Permission: `sfcore.admin.modules.joinmotd`.
- Added `/help` (aliases `/?`, `/bukkit:?`, `/bukkit:help`, `/minecraft:help`, `/minecraft:?` via the preprocess listener) — opens the SF-Core paginated GUI help menu. Permission: `sfcore.help.gui` (default true).
- Tab completion updated for both `/sf` toggles.
...
### Permissions
- New: `sfcore.admin.modules.joinleave` (default `op`).
- New: `sfcore.admin.modules.joinmotd` (default `op`).
- New: `sfcore.help.gui` (default `true`).
- Both new admin nodes added to the `sfcore.admin.*` wildcard children.
...
### API / Utilities
- New `AdventureMessageUtil` formatter at `src/main/java/dev/sergeantfuzzy/sfcore/util/message/AdventureMessageUtil.java`. Renders MiniMessage-style input via Adventure when available (Paper 1.16+); falls back to BungeeCord chat components otherwise. Supports legacy color codes, `&#RRGGBB`, `<#RRGGBB>`, named colors, gradients, hover (`<hover:show_text:'...'>`), click events, decorations, `<reset>`, and `<shadow:#RRGGBB>` (renders on 1.21.4+ Adventure path; silently skipped on legacy paths). Hex colors degrade to the nearest legacy palette entry for 1.12 clients.
- `AdventureMessageUtil.renderAnsi(raw, placeholders)` converts the same MiniMessage spans into truecolor ANSI escapes (`[38;2;R;G;Bm` plus bold/italic/underline/strikethrough/obfuscated). Hover/click annotations are dropped since terminals can't render them. Console join/leave broadcasts route through `Main.writeConsoleLine` (the same direct-console writer used by the SF-Core boot banner) so the colors actually print instead of being stripped by the standard logger.
- New `JoinLeaveService` at `src/main/java/dev/sergeantfuzzy/sfcore/storage/JoinLeaveService.java` provides typed accessors and runtime toggles for the join/leave and join-MOTD config keys.
- New paginated GUI at `src/main/java/dev/sergeantfuzzy/sfcore/gui/player/HelpGuiMenu.java` with companion holder `HelpGuiHolder.java`.

### Internal
- New listener `JoinLeaveListener` at `src/main/java/dev/sergeantfuzzy/sfcore/listener/player/JoinLeaveListener.java` handles `PlayerJoinEvent` and `PlayerQuitEvent`. Suppresses vanilla join/leave messages when the module is enabled and dispatches the configured Adventure-styled broadcast to players plus an ANSI-encoded copy to the server console. Welcome MOTD is sent on a 10-tick delay so it lands after vanilla join handling.
- New listener `HelpGuiListener` at `src/main/java/dev/sergeantfuzzy/sfcore/listener/menu/HelpGuiListener.java` cancels click/drag in the help GUI, handles pagination, runs the clicked command, and intercepts `PlayerCommandPreprocessEvent` for `/help`, `/?`, `/bukkit:help`, `/bukkit:?`, `/minecraft:help`, `/minecraft:?` so the GUI wins regardless of plugin load order.
- New executor `HelpGuiCommand` at `src/main/java/dev/sergeantfuzzy/sfcore/command/core/HelpGuiCommand.java` opens the GUI for players and falls back to a console-friendly hint.
- `Main` registers the new service, listener, and command, exposes `getJoinLeaveService()`, and reloads the service on `/sf reload`.
- `SFCoreCommand` adds `joinleave` and `joinmotd` subcommand handlers and tab completions.
...
- `src/main/java/dev/sergeantfuzzy/sfcore/storage/JoinLeaveService.java`
...
- `src/main/java/dev/sergeantfuzzy/sfcore/chat/service/ChatService.java`
- `src/main/java/dev/sergeantfuzzy/sfcore/chat/format/ChatFormatter.java`
- `src/main/java/dev/sergeantfuzzy/sfcore/chat/listener/ChatManagementListener.java`
- `src/main/resources/systems/chat_management.yml`
- `src/main/java/dev/sergeantfuzzy/sfcore/tablist/service/TablistService.java`
...
## Files Removed
- `src/main/java/dev/sergeantfuzzy/sfcore/listener/chat/ChatListener.java` — superseded by `ChatManagementListener` which subsumes the mute check, custom layout, Adventure dispatch, and ANSI console mirror.
...
- /sf joinleave + join-leave config section: Adventure-styled join, leave,
  and first-join broadcasts with gradient, hover, click, and shadow support.
- /sf joinmotd + join-motd config section: multi-line in-game welcome MOTD
