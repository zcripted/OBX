...
| `obx.ah.*` | `--` | `--` | `--` | Full auction house access (use/buy + sell + admin cancel + confirm bypass). | true | `obx.ah.*` |
...
| `obx.upkeep.exempt` | `--` | `--` | `--` | Exempts a player from the daily claim-upkeep fee (`economy.sinks.claim-upkeep`). | op | `obx.upkeep.exempt` |
| `obx.bank.tier.<name>` | `--` | `--` | `--` | Rank-based bank interest tier — matched against the `permission` nodes named in `economy.bank.tiers`; highest matching rate wins. | false | `obx.bank.tier.vip` |
| `obx.shop.*` | `--` | `--` | `--` | Full shop access (browse/buy + sell inventory + admin reload + sell wand + all categories). | op | `obx.shop.*` |
...
| `/ah` | Command: /auction, /auctionhouse | `/ah [sell <price> [category]\|auction <startingBid> [buyout] [category]\|bid <id> <amount>\|search <text>\|confirm\|mine\|claim]` | `/ah sell 250 tools` | Player **auction house** GUI with sort, category filter, and **text search** (`/ah search <text>` or in-GUI). `sell` lists the held item fixed-price; **`auction` starts a bidding listing** (optional instant **buyout**); `bid` places a bid (≥ buyout wins instantly; outbid players are auto-refunded); purchases at/above `confirm-threshold` pause for **`/ah confirm`**; `mine` manages your listings; `claim` collects proceeds and expired/cancelled items. Race-safe (one buyer per listing), money-before-goods with refunds. **Listing fees and sales tax flow to the visible server account** (`/eco server`), not burned. Staff shift-click in browse to admin-cancel any listing (`obx.ah.admin`). Config: `economy.auction.*` (max-listings, duration-hours, listing-fee, tax-percent, confirm-threshold). | true | `obx.ah` (+ `.sell`, `.admin`, `.confirm.bypass`) |
