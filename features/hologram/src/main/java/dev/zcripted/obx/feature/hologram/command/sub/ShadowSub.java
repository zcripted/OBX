...
/** {@code /holo shadow <id> <true|false>} */
public final class ShadowSub implements HoloSubCommand {

    private final HoloContext ctx;

    public ShadowSub(HoloContext ctx) {
        this.ctx = ctx;
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage("§5/holo shadow §f<id> §7<true|false>");
            return true;
        }
        ...
        hologram.getSettings().setShadow(Boolean.parseBoolean(args[1]));
